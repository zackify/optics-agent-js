...
export const reportRequestStart = (context, queryInfo, queryContext) => {
  ...
  if (!context.queries[queryContext]) {
    context.queries[queryContext] = []; // eslint-disable-line no-param-reassign
  }
  context.queries[queryContext].push({
    info: queryInfo,
...
export const reportRequestEnd = (req) => {
  ...
  try {
    ...
    (context.resolverCalls || []).forEach((resolverReport) => {
      ...
      for (const queryObj of (queries[resolverReport.resolverContext] || [])) {
    ...
    const flatQueries = [];
    queries.forEach((qList,_,__) => qList.forEach(qObj => flatQueries.push(qObj)));
