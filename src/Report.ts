...
import {hostname, platform as osPlatform, release as osRelease, arch as osArch, type as osType} from 'os';
...
import {printType, latencyBucket, newLatencyBuckets, addLatencyToBuckets, trimLatencyBuckets} from './Normalize';
import {
...
const REPORT_HEADER = new ReportHeader({
    ...
    uname: `${osPlatform()}, ${osType()}, ${osRelease()}, ${osArch()})`,
});
        ...
        Object.keys(reportData).forEach((query) => {
            ...
            Object.keys(clients).forEach((client) => {
                ...
                Object.keys(versions).forEach((version) => {
                    v.count_per_version[version] = versions[version];
                });
        ...
        queries.forEach((queryList) => {
            queryList.forEach(({info, resolvers: queryResolvers = []}) => {
                ...
                if (!res[query]) {
                    ...
                    const typeInfo = new TypeInfo(agent.schema, undefined);
